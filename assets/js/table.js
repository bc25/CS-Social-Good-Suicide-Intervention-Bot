...
function loadInterventionsTable(data) {
  console.log("MADE IT TO THE INTERVENTIONS TABLE");
  var table = document.getElementById("interventionsTable").getElementsByTagName('tbody')[0];
  if (data.length < 1) {
    document.getElementById("interventionsTable").setAttribute("style", "display:none;");
    var header = document.getElementsByClassName("header")[0];
    header.getElementsByTagName("h4")[0].innerHTML = "No Active Interventions";
    header.getElementsByTagName("p")[0].innerHTML = "You currently have no active interactions.";
    return;
  }
  for (var i = 0; i < data.length; i++){
    ...
    var parsedUserID = data[i]["profile_url"].substring("http://stanford.ipbhost.com/profile/".length);
    var messageLink = "http://stanford.ipbhost.com/messenger/compose/?to=" + parsedUserID;
    cell3.innerHTML = "<a type=\"button\" class=\"btn btn-primary\" href=\""+messageLink+"\">Message</a>";
    cell3.innerHTML += "<button type=\"button\" class=\"btn btn-primary\" data-toggle=\"modal\" data-target=\"\#myDone\" data-id=\"" + data[i]["_id"] + "\">Done</button>";
...
function interventionInfo(content, entry) {
  ...
  */
  ...
  complete_date.innerHTML = "<b>Date Completed:</b> " + entry["complete_date"];
